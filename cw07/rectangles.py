...
class Rectangle:
    ...
    def __hash__(self):
        return hash(self.pt1) ^ hash(self.pt2)
    ...
    def __eq__(self, other):
        if not isinstance(other, Rectangle):
            raise ValueError("That's not a rectangle!")
...
class TestRectangle(unittest.TestCase):
    ...
    def test_intersection(self):
        ...
        with self.assertRaises(ValueError):
            self.r7.intersection("Rectangle(1, 2)")
        with self.assertRaises(ValueError):
            self.r3.intersection(Point(2, 5))

    def test_cover(self):
        self.assertEqual(self.r2.cover(self.r3), Rectangle(0, 0, 4.5, 4.5))
        self.assertEqual(self.r5.cover(self.r6), Rectangle(-8, -5, 5, 5))
        self.assertEqual(self.r4.cover(self.r7), Rectangle(-2, 0, 8.5, 10.25))
        with self.assertRaises(ValueError):
            self.r7.cover("str")
        with self.assertRaises(ValueError):
            self.r3.cover(set((2, 5)))

    def test_make4(self):
        s1 = set([Rectangle(0.0, 0, 1, 1), Rectangle(1, 0, 2, 1),
                  Rectangle(0, 1.0, 1, 2), Rectangle(1, 1, 2, 2)])
        s2 = set([Rectangle(-2, 0, 1, 2), Rectangle(1, 0, 4, 2),
                  Rectangle(-2, 2, 1, 4), Rectangle(1, 2.0, 4, 4)])
        self.assertEqual(set(self.r2.make4()), s1)
        self.assertEqual(set(self.r4.make4()), s2)
